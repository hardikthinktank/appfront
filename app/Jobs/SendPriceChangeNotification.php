...
class SendPriceChangeNotification implements ShouldQueue
{
    ...
    // {
    //         Mail::to($this->email)->send(new PriceChangeNotification($this->product,$this->oldPrice,$this->newPrice));

    // }
    public function handle()
    {
        Log::info("SendPriceChangeNotification job started for product ID: {$this->product->id}");
